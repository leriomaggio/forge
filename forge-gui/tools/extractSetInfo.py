...
pathToForgeBoosters = os.path.join(
    sys.path[0], "..", "res", "blockdata", "boosters.txt"
)


class CardInSet:  # CardInSet
    def __init__(self, name, rarity=None):
        self.rarity = "C" if rarity is None else rarity
        self.name = name

    def __str__(self):
        return self.name + " | " + self.rarity_full()

    def __repr__(self):
        return self.__str__()

    def rarity_full(self):
        if self.rarity == "B":
            return "Land"
        if self.rarity == "R":
            return "Rare"
                ...
                if line == "\n":
                    ...
                    for i in range(len(sets)):
                        ...
                        for x in range(0, prints):
                            sc.append(CardInSet(tmpName, ee[1].strip()))
            prevline = line
    ...
    for setCode in setsMtgData:
        ...
        with open(sys.path[0] + os.sep + "sets" + os.sep + fileName, "w") as output:
            ...
            for cs in mtgData["Cards"]:
                output.write(cs.rarity_short() + " " + cs.name + "\n")
